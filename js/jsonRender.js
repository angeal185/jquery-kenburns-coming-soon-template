						...
						case 'html':
							...
							if(jsonRender._isArray(_transform)) {
								...
								switch(typeof temp){
									...
									//Only returned by jsonRender.transform or $.jsonRender calls
									case 'object':
										//make sure this object is a valid jsonRender response object
							...
							if( key.length > 2 )
								if(key.substring(0,2).toLowerCase() == 'on') {
									...
									if(options.events) {
										...
										//Insert temporary event property (jsonRender-event-id) into the element
...
(function($){	
    ...
    //Alias for jsonRender.transform
	// _options 
	//   output : jsonRender | html | jquery
    $.jsonRender = function(json, transform, _options) {
        
        //Make sure we have the jsonRender base loaded
		...
		switch(options.output){
			...
			// for consumption within a jsonRender html attribute function call 
			...
			//Return raw html (same as calling jsonRender.transform
			case 'html':
			...
			case 'jquery':
				...
				//let jsonRender core do it's magic
	...
	$.fn.jsonRender = function(json, transform, _options) {
        ...
        //Otherwise we're running $().jsonRender
